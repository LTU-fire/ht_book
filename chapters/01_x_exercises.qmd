...
Assume an insulated steel pipe with a liquid medium of the temperature, $T_{in} = 100°C$. The pipe is exposed from the outside by a temperature, $T_f$ = 800°C. The inner heat transfer coefficient, $h_{in}$ = 100 W/m²K for the exposed outside, $h_out$ = 50 W/m²K. The conductivity of steel, $\lambda_s$ = 45 W/mK and the conductivity of the insulation, $\lambda_i$ = 0.5 W/mK. Calculate the temperature of the steel pipe, $T_1$.
...
$$
R_{tot} = R_{in} + R_{s} + R_{i} + R_{o} = \frac{1}{2\pi}\left(\frac{1}{r_i h_{in}} + \frac{\ln\left(r_{o,s}/r_{i,s}\right)}{\lambda_{s}} + \frac{\ln\left(r_{o,i}/r_{i,i}\right)}{\lambda_{i}} + \frac{1}{r_i h_{in}}\right) \approx 0.41 \text{ } m²K/W
$$

As $T_in$ = 100°C and $T_f$ = 800°C, the temperatures at the surface, $T_1$, can be calculated as

$$
T_1 = T_in + \frac{R_{in}}{R_{tot}} \left(T_f - T_in \right) = 
75 + \frac{\tfrac{1}{100}}{0.41} \left(800 - 100 \right) = 197.3°C
$$

An alternative way to calculate the surface temperature is by using

$$
T_1 = \frac{R_h T_2 + \left(R_s + R_i + R_o\right) T_g}{R_{tot}} = 197.3°C
$$
